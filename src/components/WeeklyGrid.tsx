...
import type { Course, CourseEvent, Day, TimeHM } from "../types";
...
export function WeeklyGrid(props: {
  ...
  const dayList = useMemo(
    () => computeDayList(selectedCourses, semester),
    [selectedCourses, semester]
  );

  const minMin = parseHM(timeRange.min as TimeHM);
  const maxMin = parseHM(timeRange.max as TimeHM);
  ...
  const perDay = useMemo(() => {
    const map: Record<
      Day,
      { scheduled: GridEvent[]; unscheduled: GridEvent[] }
    > = {} as any;
    ...
    for (const course of selectedCourses) {
      ...
      for (const ev of course.events) {
        ...
        if (!canScheduleEvent(ev, store.scheduled, store.courses, semester))
          continue;
    ...
    <div className="h-full w-full bg-white dark:bg-slate-900 border dark:border-slate-700 rounded-lg p-2 overflow-auto">
      ...
      >
        ...
        {dayList.map((day) => (
          <div
            key={`hdr-${day}`}
            className="border-b dark:border-slate-700 text-sm font-medium px-2 py-1 text-center"
          >
            {t(`days.${day}`)}
          </div>
        ))}
            ...
            {ticks.map((m) => {
              ...
              return (
                <div
                  ...
                  style={{ top: `${top}%` }}
                >
                  <div className="text-xs text-slate-500 dark:text-slate-400 select-none -translate-y-2 px-1">
                    {hh}:00
                  </div>
                </div>
              );
        ...
        {dayList.map((day) => (
          <DayColumn
            ...
            onClickSchedule={(course, event) =>
              store.scheduleEvent(course.id, event.category, event)
            }
            onClickUnschedule={(course, event) =>
              store.unscheduleEvent(course.id, event.category)
            }
